...
| FP-03 | Visibilidad numérica en pantalla | Panel compacto `VisibilityReadoutPanel` en esquina superior derecha (`PRESET_TOP_RIGHT`, offsets −190/18/−12/50). Muestra el valor de `FPVisibilityOverlay.format_visibility()` (e.g. `Vis FP 4.8m`) independientemente del overlay técnico. Controlado por `@export var show_visibility_readout: bool = true` (grupo `Technical Overlay`). Usa `visibility_label` ya calculado en `_update_status_hud()` — sin recalcular nada. | **Alta** | ✅ commit actual |
