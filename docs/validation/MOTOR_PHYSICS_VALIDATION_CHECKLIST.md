...
| Mandatory independent motor audit | USER STOP / BLOCKING | After H3.2b1b closes, notify the user and stop before implementing H3.2b4 or H3.3. Audit architecture, state ownership, tick ordering, diagnostics, tests and validation evidence with an independent adversarial reviewer. Continuation requires explicit user authorization after reviewing the audit. |
