...
- **O2E1 Thornton cross-check added (WARN, commit 90c436a)** — added cross-subsystem rule O2E1 comparing `o2_consumed_kg_total_all` (OES all-paths accumulator) against `hrr_kj_total * 7.6e-5 kg/kJ` (CombustionSystem tracking-only). Instrumentation: new `room.hrr_kj_total` tracking field in RoomModel, accumulated in CombustionSystem, exported via StateBuilder/LogWriter. Corpus audit (2026-06-27, 11 CSVs): 8 WARN, 3 PASS (old-schema graceful skip), 0 FAIL, 1308 WARN findings. Root cause: in standard two-zone mode OES accumulates Thornton O2 in both the bulk path (line 362) and the upper-zone path (line 407), producing `o2_consumed_kg_total_all ≈ 2 × Thornton`. Max residual 1.95e-5 kg. O2E1 remains WARN; promotion to FAIL requires fixing the double-accounting and re-auditing. 155 unit tests PASS. Guardrails: 349/354 stable.
