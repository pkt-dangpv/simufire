...
| SF-AUD-009 | Estratificacion | **[DIAGNOSTICADO 2026-05-14, FIX PENDIENTE]** La estratificacion combina masa de humo, gas superior y relajaciones; no es un modelo de zona completo. Causa raiz identificada: cuando `l_flame >= room_height` (fuegos >~700 kW en sala de 2.4 m), el termino McCaffrey clampea `z_eff=0.1 m` permanentemente, dando entrainment ~0.03 kg/s en vez del fisico ~2.4 kg/s (84x menor). Fix intentado (revertido): rama confined con `z_eff=thermal_layer_m`. El fix activa el regimen desde t=0 para todos los fuegos grandes, rompiendo 7/17 baselines calibrados. La correccion real requiere separar el caso `l_flame < room_height` (far-field, formula actual) del caso `l_flame >= room_height` (fuego que llena el compartimento, z_eff=height_m) y recalibrar baselines afectados. La capa llega al suelo por dos mecanismos independientes: (1) acumulacion de masa en zona superior por McCaffrey, (2) expansion termica del gas superior al calentarse (a 500 C el mismo gas ocupa el 76% mas de volumen que a 100 C). El fuego a menudo se extingue por O2 antes de que ocurra ninguno de los dos. | `ThermalSystem.gd:424-450` (bloque McCaffrey, z_eff clamp). | CFAST TN 1889v1; McCaffrey NBSIR 79-1910; Heskestad (1983) | Critica | Interfaz no desciende al suelo en fuegos grandes como en incendios reales. | Separar rama far-field (l_flame < thermal_layer_m) de rama confined (l_flame >= room_height) con z_eff=height_m; recalibrar baselines con nueva dinamica. |
...
| SF-AUD-012 | Flashover | **[PARCIALMENTE CORREGIDO 2026-05-14]** Añadido criterio de flujo radiante al suelo: `q_floor = ε·σ·T_upper⁴` calculado cada paso y exportado en state/CSV como `floor_heat_flux_kw_m2`. Flashover se activa si `q_floor >= 20 kW/m²` (ISO 9705) AND HRR suficiente AND capa baja, como criterio alternativo (OR) al de temperatura. La ignicion generalizada de superficies (`generalize_room_combustion_after_flashover`) ya existia. Pendiente: criterio Thomas/MQH explícito como predictor de onset, view factors por sala y validacion con ISO 9705/room corner. | `SimulationEngine.gd:309-325` (params), `SimulationEngine.gd:1584-1649` (_try_trigger_flashover), `RoomModel.gd` (floor_heat_flux_kw_m2), `CombustionSystem.gd:755` (generalize). | ISO 9705; NFPA 921; SFPE flashover criteria | Alta (antes Critica) | Flujo al suelo ya calculado y como criterio. Sin Thomas/MQH ni view factors geometricos. | Añadir Thomas/MQH como predictor; validar con ISO 9705/room corner. |
