...
var combustion_system = CombustionSystemScript.new()
...
@export var smoke_settling_base_per_s: float = 0.0004
@export var smoke_settling_bonus_per_s: float = 0.0060
@export var co_postfire_purge_base_per_s: float = 0.0002
@export var co_postfire_purge_bonus_per_s: float = 0.0030
...
func _step_oxygen(dt: float) -> void:
	...
	for op in building.get_openings():
		...
		if op.a == BuildingModel.OUTSIDE_ID or op.b == BuildingModel.OUTSIDE_ID:
			...
			var h_drive_int: float = float(flow_state.get("h_drive_m", 0.0))
			...
			if h_drive_int <= 0.0 or area_eff <= 0.0 or engagement <= 0.0:
				continue

			var q: float = 0.65 * 0.5 * area_eff * sqrt(g_gravity * h_drive_int * delta_t_k / ((t_hot_k + t_cold_k) * 0.5))
			var exch: float = q * air_density_kg_m3 * dt * doorway_o2_exchange_coeff * engagement
			...
			var max_exch: float = minf(mass_hot, mass_cold) * 0.08
			exch = minf(exch, max_exch)
			if exch <= 0.0:
				continue
...
func _step_pressure_venting(dt: float) -> void:
	...
	for room_id in building.get_rooms().keys():
		...
		var h_smoke_m: float = maxf(0.0, room.height_m - _effective_hot_layer_height_m(room))
...
func _step_smoke(dt: float) -> void:
	...
	for room_id in building.get_rooms().keys():
		...
		if cleanup_factor > 0.0:
			...
			smoke_deposited_total_kg += deposited_smoke_kg
...
func _compute_postfire_cleanup_factor(room: RoomModel) -> float:
	...
	var pressure_span_pa: float = maxf(
		0.01,
		postfire_cleanup_pressure_stop_pa - postfire_cleanup_pressure_full_pa
	)
...
func get_state() -> Dictionary:
	...
	for room_id in building.get_rooms().keys():
		...
		state[str(room_id)] = {
			...
			"temp_at_1_8m_c": _estimate_temperature_at_height_m(room, 1.8),
			"temp_at_1_5m_c": _estimate_temperature_at_height_m(room, 1.5),
			"temp_at_1_1m_c": _estimate_temperature_at_height_m(room, 1.1),
			...
			"fuel_objects_remaining_MJ": combustion_system.get_room_total_remaining_fuel_MJ(room),
			"fuel_objects_max_hrr_kw": combustion_system.get_room_total_max_hrr_kw(room),
			"fuel_objects_active_count": combustion_system.get_room_active_object_count(room),
			"co_ppm": _compute_co_ppm(room),
			"is_quiescent": _is_room_quiescent(room),
...
func _append_log_snapshot() -> void:
	...
	for room_id in building.get_rooms().keys():
		...
		var co_ppm_log: float = _compute_co_ppm(room)
		var hot_layer_m: float = _effective_hot_layer_height_m(room)
		var layer_150c_m: float = clampf(room.layer_150c_m, 0.0, room.height_m)
