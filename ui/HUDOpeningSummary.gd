...
static func compact_label(summary: Dictionary) -> String:
	...
	var state_short: String = "RT" if bool(summary.get("glass_broken", false)) else ("AB" if open_pct >= 95 else ("CE" if open_pct <= 5 else "%d%%" % open_pct))
...
static func compact_color(summary: Dictionary) -> Color:
	...
	if bool(summary.get("glass_broken", false)):
		return SimuFireThemeScript.ORANGE
